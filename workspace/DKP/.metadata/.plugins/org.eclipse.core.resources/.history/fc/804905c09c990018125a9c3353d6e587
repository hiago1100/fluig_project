var BDOForms = {
    ...
    $(document).on('ZoomSecaoSelecionadaDois', function(ev, retorno) {
          window.loadingLayer.show();
          setTimeout(function() {
            console.log("teste 123");
           $("#cpObra").val(retorno.SECAO);

           console.log(retorno.CODCOLIGADA);
           console.log(retorno.SECAO);
    ...
    $(document).on('ZoomColaboradorSelecionado', function(ev, colaborador) {
        ...
        setTimeout(function() {
            ...
            if (FLUIGC.sessionStorage.getItem('selecionandoOutorgado')) {
                ...
                console.log("teste nome "+ dados2.values[0].NOME);
                // Compartilhados.LimparCampos(['limparDadosColaboradorSubstituto']);

                 $('#cpNomeOutorgado').val(dados2.values[0].NOME);

                 //$('#cpEstadoCivil').val(dados2.values[0].NOME);
                 
                 $('#cpCpf').val(dados2.values[0].CPF);
                // $('#cpRg').val(dados2.values[0].RG);
                // $('#cpFuncao').val(dados2.values[0].FUNCAO);
                // $('#cpNacionalidade').val(dados2.values[0].NACIONALIDADE);
                // $('#cpCep').val(dados2.values[0].CEP);
                // $('#cpLogradouro_dois').val(dados2.values[0].LOGRADOURO);
                // $('#cpNumero_dois').val(dados2.values[0].NUMERO);
                // $("#cpComplemento_dois").val(dados2.values[0].COMPLEMENTO);
                // $("#cpBairro_dois").val(dados2.values[0].BAIRRO);
                // $("#cpCidade_dois").val(dados2.values[0].CIDADE);
                // $("#cpBairro_dois").val(dados2.values[0].BAIRRO);
