/**
 * @name Compra de Materiais ou Contratação de Serviços de Terceiros
 * @description Fluxo de Compra de Materiais ou Contratação de Serviços
 * @author Mateus Vidal
 * @modifier Mateus Vidal
 * @version v1.1.1 (feature.task.fix)
 * @lastmod 27/08/2019
 */
...
const View = (function(){
	...
	function startInstance() {
		...
		/**
		 * @description Método inicializador da classe View
		 */
		...
		function renderMustache(){
			let $cardBody = $('#sec-footer');
			let template;
			let rendered;

			template = $('#tpl_footer').html();
			Mustache.parse(template);   // optional, speeds up future uses
			rendered = Mustache.render(template, FORMDOC);
			...
			if(tipo_solicitacao == 'itens_ti'){
				...
				for(let i = 1; i <= index_tabela; i++){
					...
					json_orcado = json_orcado.filter(function(item){
						if(item.INSUMO_PLANEJAMENTO == $('#cpCodInsumoPlanejItensTi___' + i).val()){
							return item;
						}
					});
		...
		function bindZoomEvent(cod_usuario_rede){
			$(document).on('callbackZoomEmpresa', function(ev, dados){
				let retorno_obra_uau;

				$('#cpEmpresa').val(dados.EMPRESA);
				$('#cpCnpjEmpresa').val(dados.CNPJ);
				$('#cpCodEmpresa').val(dados.CODEMPRESA);

				retorno_obra_uau = getObraUAUPopulandoZoom(cod_usuario_rede, dados.CODEMPRESA);

				Zoom.createZoomCentroCustoUAU(retorno_obra_uau);
			});

			$(document).on('callbackZoomCentroCustoUAU', function(ev, dados){
				$('#cpCentroCustoUau').val(dados.DESCRICAOOBRA);
				$('#cpCodCentroCustoUau').val(dados.CODOBRA);
			...
			$(document).on('callbackAutoCompleteInsumoComposicao', function(ev, dados){
				let tipo_solicitacao = FLUIGC.sessionStorage.getItem('tipo_solicitacao');
				...
				switch(tipo_solicitacao){
					case 'demais_compras':
						//all code
						$(`#cpCodInsumoDemaisCompras___${index_tabela}`).val(dados_insumo.CODIGO);
						$(`#cpCAPDemaisCompras___${index_tabela}`).val(dados_insumo.CAP);
						$(`#cpUnidadeDemaisCompras___${index_tabela}`).val(dados_insumo.UNIDADE);

						break;
					case 'itens_ti':
						//all code
						$(`#cpCodInsumoItensTi___${index_tabela}`).val(dados_insumo.CODIGO);
						$(`#cpCAPItensTi___${index_tabela}`).val(dados_insumo.CAP);
						$(`#cpUnidadeItensTi___${index_tabela}`).val(dados_insumo.UNIDADE);
						break;
					case 'servicos':
						//all code
						$(`#cpCodInsumoServicos___${index_tabela}`).val(dados_insumo.CODIGO);
						$(`#cpCAPServicos___${index_tabela}`).val(dados_insumo.CAP);
						$(`#cpUnidadeServicos___${index_tabela}`).val(dados_insumo.UNIDADE);
						break;
				}
			});
			...
			$(document).on('callbackZoomProdutoUAU', function(ev, dados){
				...
				switch(tipo_solicitacao){
					...
					case 'servicos':
						...
						$(`#cpCodProdutoPlanServicos___${index_tabela}`).val(dados.CODPRODUTO);

						$(`#btComposicaoPlanejServicos___${index_tabela}`).trigger('click');
						break;
				}
			});
		...
		function bindFormEvents(cod_usuario_rede){
			...
			$('#btCentroCustoUau').click(function(){
				if($('#cpEmpresa').val() != '') {
					...
					Zoom.createZoomCentroCustoUAU(dados);
		...
		function addLinhaTabelaServicos(){
			...
			if(qtde_linhas_tabela <= 30){
				...
				$(`#btComposicaoPlanejServicos___${index_tabela}`).click(function() {
					let cod_empresa = $('#cpCodEmpresa').val();
					let cod_obra = $('#cpCodCentroCustoUau').val();
					let cod_produto = $(`#cpCodProdutoPlanServicos___${index_tabela}`).val();
					let mes_corrente = moment().format('MM');
					let ano_corrente = moment().format('YYYY');

					if(cod_produto != ''){
						FLUIGC.sessionStorage.setItem('zoomPlanejamentoUAUServicos', true);
						Zoom.createZoomPlanejamentosUAU(cod_empresa, cod_obra, cod_produto, mes_corrente, ano_corrente, index_tabela);
					}
					else{
						parent.FLUIGC.toast({
		...
		function fillTabelaPlanejamentoOrcamento(index, json_pedido, tabela){
			Dal.getPedidoCompraUAU(function(status, fornecedor){
				if(status){
					$('.forn-aprovado').show();
					$('#cpDataCotacao').val(moment().format('DD/MM/YYYY'));

					switch(tabela){
						case 'DemaisCompras':
							wdkAddChild('tblAnaliseVerbaDemaisCompras');
							...
							$(`#cpSaldoDemaisCompras___${index}`).val(Bll.calcSaldoOrcamento(index, tabela));

							break;
						case 'ItensTi':
							wdkAddChild('tblAnaliseVerbaItensTI');
							...
							$(`#cpSaldoItensTi___${index}`).val(Bll.calcSaldoOrcamento(index, tabela));

							break;
						case 'Servicos':
							wdkAddChild('tblAnaliseVerbaServicos');
							...
							$(`#cpSaldoServicos___${index}`).val(Bll.calcSaldoOrcamento(index, tabela));
