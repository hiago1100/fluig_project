...
var ARYForms = {
		...
		onView: function(params) { //Visualização do formulário sem a possibilidade de edição (consulta)
				...
				$(".mostrarConteudo").removeClass('hide');
		...
		onEdit: function(params) {  //Edição do formulário;
			...
			var hideSuper = $("#verificaSuper").val();
...
//				$("#totalMercadoria___"+linha+"',#totalitem___"+linha+"',#bcicms___"+linha+"',#valorIpi___"+linha+"',#valorst___"+linha).maskMoney({showSymbol:true, symbol:"R$", decimal:",", thousands:"."}); 
				...
				var alcada_repAux = $("#alcadaAux").val();

				if (WKNumState == 0 && hideSuper == "SIM") {
				...
				if (parseFloat(valorNfd) >= parseFloat(alcada_repAux)) {
					...
					console.log("MINHA NOVA TENTATIVA");
	...
	//			var toleraMercAux = $("#toleraMerc").val() ; 
				var toleraMerc   =  $("#toleraMerc").val(); 
				
				console.log("toleraMerc ", toleraMerc);


				var porcMerc     = $("#porcMerc").val() ;								
				//var porcMerc        = porcMerc.toFixed(2);

				console.log("porcMerc ", porcMerc);

				var toleraTotal  = $("#toleraTotal").val() ;
				//var toleraTotal     = toleraTotal.toFixed(2) ;

				console.log("toleraTotal ", toleraTotal);
				
				var porcTotal    = $("#porcTotal").val() ;
				//var porcTotal       = porcTotal.toFixed(2 ) ;

				console.log("porcTotal ", porcTotal);


				var toleraImp    = parseFloat($("#toleraImp").val()) ;
				//var toleraImp    =  toleraImp.toFixed(2) ;

				console.log("toleraImp ", toleraImp);

				var porcImp      = $("#porcImp").val() ;
				//var porcImp      	= porcImp.toFixed(2) ;

				console.log("porcImp ", porcImp);
				...
				var icms_calcImpAux = icms_calcImp.toFixed(2);

		//		console.log("VALOR CAMPO IMPOSTO ICMS", icms_calcImpAux);

				var result_icmsImp = (parseFloat(porcImp) * parseFloat(icms_calcImpAux)) /100;
				var result_icmsImpAux = result_icmsImp.toFixed(2);

			//	console.log("VALOR RESULTADO PERC ICMS", result_icmsImpAux); // esse resultado esta errado esta sem virguma



				console.log("VALOR  difIcms "+ difIcms.toFixed(2) +  " / "+ toleraImp );

				difIcms = parseFloat(difIcms).toFixed(2);

				

				if (parseFloat(difIcms) <= 0){
					//console.log(" IF 1 ICMS");
					$('#TotalDifGetalIcms').css('background-color','#87CEEB');
					$("#controle1").val("ok");
					$("#corGeralIcms").val("azul");
				}

				if (parseFloat(difIcms) != 0 && parseFloat(difIcms) <= result_icmsImpAux && parseFloat(difIcms) > 0 && result_icmsImpAux > 0){
					//console.log(" IF 2 ICMS");
				...
				if(parseFloat(difIcms) != 0 && parseFloat(difIcms) <= toleraImp && parseFloat(difIcms) > 0){
					//console.log(" IF 3 ICMS");
					console.log("toleraImp: "+ toleraImp);
					$('#TotalDifGetalIcms').css('background-color','#F0E68C');
					$("#controle1").val("ok");
					$("#corGeralIcms").val("amarelo");
				...
				if (parseFloat(difIcms) > result_icmsImpAux && result_icmsImpAux > 0){ 
					//console.log(" IF 4 ICMS");
					$('#TotalDifGetalIcms').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralIcms").val("vermelho");

				}if(parseFloat(difIcms) > parseFloat(toleraImp)){
					//console.log(" IF 5 ICMS");
					$('#TotalDifGetalIcms').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralIcms").val("vermelho");
				}
...
//				imposto IPI
				...
				var ipi_calcImp = parseFloat($('#TotalDescGetalIpi').val());
				var ipi_calcImpAux = ipi_calcImp.toFixed(2);

				var result_ipiImp = (porcImp * parseFloat(ipi_calcImpAux)) /100;
				
				var result_ipiImpAux = result_ipiImp.toFixed(2);

				var result_ipiVlr = parseFloat($('#TotalGetalIpi').val()) + toleraImp;


				difIpi = parseFloat(difIpi).toFixed(2);
				...
				if (parseFloat(difIpi) <= 0){
				 ...
				 if (parseFloat(difIpi) != 0 && (parseFloat(difIpi) <= result_ipiImpAux) && (parseFloat(difIpi) > 0) && result_ipiImpAux > 0 ){
				 ...
				 if (parseFloat(difIpi) != 0 && (parseFloat(difIpi) <= toleraImp) && (parseFloat(difIpi) > 0)){
					$('#TotalDifGetalIpi').css('background-color','#F0E68C');
					$("#controle2").val("ok");
					$("#corGeralIpi").val("amarelo");
				 ...
				 if (parseFloat(difIpi) != 0 && (parseFloat(difIpi) > result_ipiImpAux) && result_ipiImpAux > 0 ){
					$('#TotalDifGetalIpi').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralIpi").val("vermelho");			

				} if ((parseFloat(difIpi) > toleraImp)) {
					$('#TotalDifGetalIpi').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralIpi").val("vermelho");		
...
//				imposto ST
				...
				var st_calcImp = parseFloat($('#TotalDescGetalSt').val());
				var st_calcImpAux = st_calcImp.toFixed(2);

				var result_StImp = (porcImp * parseFloat(st_calcImpAux)) /100;
				var result_StImpAux = result_StImp.toFixed(2);

				var result_StVlr = parseFloat($('#TotalGetalSt').val()) + toleraImp;


				//difSt = parseFloat(difSt).toFixed(2);


				console.log("VALOR DIFERENCA ST = ", parseFloat(difSt));

				if(result_StImpAux == 0){
					...
					//	console.log("IF 1");
						...
						//console.log("Passou no azul");

					} 
					 if (parseFloat(difSt) != 0 &&  (parseFloat(difSt) <= result_StImpAux) && (parseFloat(difSt) > 0) && result_StImpAux > 0){

					//	 console.log("IF 2 AQUI");

						$('#TotalDifGetalSt').css('background-color','#F0E68C');
						$("#controle3").val("ok");
						$("#corGeralSt").val("amarelo");
					//	console.log("Passou no Amarelo");

					} 
					if ((parseFloat(difSt) <= toleraImp) && parseFloat(difSt) > 0){
						//console.log("IF 3");
						$('#TotalDifGetalSt').css('background-color','#F0E68C');
						$("#controle3").val("ok");
						$("#corGeralSt").val("amarelo");
					...
					if (parseFloat(difSt) != 0 && (parseFloat(difSt) > result_StImpAux) && parseFloat(result_StImpAux) > 0){ 

					//	 console.log("difSt = ",parseFloat(difSt)," result_StImpAux = ",result_StImpAux);
						// console.log("IF 4");

						$('#TotalDifGetalSt').css('background-color','#E9967A');
						$("#possuiDif").val("sim");
						$("#corGeralSt").val("vermelho");	
						//console.log("Passou no vermei");		

					} 
					if (parseFloat(difSt) > toleraImp && result_StImpAux > 0){

				 //	console.log("difSt = ",difSt," toleraImp = ",toleraImp);					
				 	//console.log("IF 5");
						$('#TotalDifGetalSt').css('background-color','#E9967A');
						$("#possuiDif").val("sim");
						$("#corGeralSt").val("vermelho");	
...
//				valor total Merc
				...
				var vlMerc_calcImp = parseFloat($('#TotalDescGetalMerc').val());
				var vlMerc_calcImpAux = vlMerc_calcImp.toFixed(2);

				//console.log("TOTAL GETA MERC : "+ vlMerc_calcImpAux);

				var result_VlMercImp = (porcMerc * parseFloat(vlMerc_calcImpAux)) /100;
				var result_VlMercImpAux = result_VlMercImp.toFixed(2);


				var result_VlMercVlr = parseFloat($('#TotalGetalMerc').val()) + parseFloat(toleraMerc);
				
				difMerc = parseFloat(difMerc).toFixed(2);

				if (parseFloat(difMerc) <= 0 ) {
					//console.log(" IF 1 difMerc "+ difMerc);
				 ...
				 if (parseFloat(difMerc) != 0 && parseFloat(difMerc) <= result_VlMercImpAux && parseFloat(difMerc) > 0 && result_VlMercImpAux > 0){
				 	//	console.log(" IF 2 difMerc "+ difMerc + " / "+ result_VlMercImpAux);
					$('#TotalDifGetalMerc').css('background-color','#F0E68C');
					$("#controle4").val("ok");
					$("#corGeralMerc").val("amarelo");
					//console.log(" VALOR DA DIFERENCA : "+ difMerc +" /  toleraMerc / " + parseFloat(toleraMerc));

				}if (parseFloat(difMerc) != 0 && parseFloat(difMerc) <= toleraMerc && parseFloat(difMerc) > 0){
						//console.log(" IF 3 difMerc "+ difMerc " / "+ toleraMerc);
					$('#TotalDifGetalMerc').css('background-color','#F0E68C');
					$("#controle4").val("ok");
					$("#corGeralMerc").val("amarelo");				
				...
				if (parseFloat(difMerc) != 0 && parseFloat(difMerc) > result_VlMercImpAux && result_VlMercImpAux > 0){
					//console.log(" ENTROU NO IF");
					$('#TotalDifGetalMerc').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralMerc").val("vermelho");

				} if (parseFloat(difMerc) > toleraMerc) {
					$('#TotalDifGetalMerc').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralMerc").val("vermelho");
...
//				valor TOTAL TOTAL
				...
				var vlTotal_calcImp = parseFloat($('#TotalDescGetalNFD').val());
				var vlTotal_calcImpAux = vlTotal_calcImp.toFixed(2);



				//console.log("VALOR TOAL NFD "+ vlTotal_calcImpAux);
				
				var result_VlTotalImp = (porcTotal * parseFloat(vlTotal_calcImpAux)) /100;
				var result_VlTotalImpAux = result_VlTotalImp.toFixed(2);


			//	var result_VlTotalVlr = parseFloat($('#TotalGetalMerc').val()) + parseFloat(toleraTotal);

				difNfd = parseFloat(difNfd).toFixed(2);
				...
				if (parseFloat(difNfd) <= 0){
				 ...
				 if (parseFloat(difNfd) != 0 && (parseFloat(difNfd) <= result_VlTotalImpAux) && parseFloat(difNfd) > 0 && result_VlTotalImpAux > 0){
				...
				} if (parseFloat(difNfd) != 0 && (parseFloat(difNfd) <= toleraTotal) && parseFloat(difNfd) > 0){
					console.log("If 3");
					$('#TotalDifGetalNFD').css('background-color','#F0E68C');
					$("#controle5").val("ok");
					$("#corGeralTotal").val("amarelo");
				 ...
				 if (parseFloat(difNfd) != 0 && (parseFloat(difNfd) > result_VlTotalImpAux) && result_VlTotalImpAux > 0){
					console.log("If 4");
					$('#TotalDifGetalNFD').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralTotal").val("vermelho");
				} if ((parseFloat(difNfd) > toleraTotal)) {
					console.log("If 5");					
					$('#TotalDifGetalNFD').css('background-color','#E9967A');
					$("#possuiDif").val("sim");
					$("#corGeralTotal").val("vermelho");
...
//				INICIO
				...
				if(hideSuper == "SIM"){
					...
					var matriculaSolicitante = $("#matriculaSolicitante").val();
						carregaAlcada(matriculaSolicitante);



						console.log("supervisor atividade 0 : ", hideSuper);
			...
			if(WKNumState == 42) {
				...
				var i = 0;
				$("#tabelaNF tbody tr:not(:first-child)").each(function(){
					i++;
					var input = $(this).find("#cod_item___"+i);
					var val = input.val();

					console.log(input + " / " +val);
					if(val == "" || val == null || val == undefined || val == " " ){
						input.closest("tr").find("td.bpm-mobile-trash-column img").click();
					}
				});
				...
				$.ajax({
					...
					success:function(model) {
						...
						$.each(model.content.values, function(index, value){
							...
							var dateFim = value.DAT_FIM_VALID; 
							var dataComeco = value.DAT_INIC_VALID;

							var dataFimAux    = dateFim.replaceAll("-","");
							var dataComecoAux = dataComeco.replaceAll("-","");
							var dataHojeAux   = data();


							console.log("SUBSTITUTO :", substituto); 
							console.log("DATA INICIO", dataComecoAux); 
							console.log("DATA FIM", dataFimAux); 
							console.log("DATA ATUAL", dataHojeAux);
							
							if (cod_mtr_fluigAux == cod_solicitante && dataComecoAux <= dataHojeAux && dataFimAux >= dataHojeAux) {			
...
function data(){
  ...
  console.log("Jonathan console:", date);
...
//		AQUI É A PARTE DOS ITENS  /\ 
	...
	}  if (selectedItem.inputId == "numeroNotas"){
