...
function setSelectedZoomItem(selecao) {
	...
	if (selecao.inputId == "funcionario") {
		...
		getColleagueByCpf(selecao.CPF).done(function (data) {
			...
			console.log("Dados retorno RM", data);
	...
	if (selecao.inputId == 'id_local_treinamento') {

		console.log(selecao.nmEndereco);
		console.log(selecao.nmEstado);
		$("#enderecoLocalTreina").val(selecao.nmEndereco);
		$("#localTreinamentoUf").val(selecao.nmEstado);
	}
}
...
$(document).ready(function () {
			...
			data: function (params) {
				...
				var dadosOperadores = {
					...
					"constraints": [{ //constraints to filter the search, all fields specified inside are required
						...
						"_initialValue": $("#" + uf).val(), //value to be filtered
						"_finalValue": $("#" + uf).val(), //final value to be filtered
						"_type": 1, //type of the constraint (1 - MUST, 2 - SHOULD, 3 - MUST_NOT)
						"_likeSearch": false
					},
					{
	...
	$('.treinamento').on('click', function () {
		$("#uftabela").val('localTreinamentoUf');
	});

	$('.localTrabalho').on('click', function () {
		$("#uftabela").val('localTrabalhoUf');
	});
