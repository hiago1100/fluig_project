	...
	try {
		...
		  if(bloqueio == "on"){
			...
			for (var j = 0; j < indices.length; j++) {
				...
				log.info('@@@ 2 idusuario: ' + hAPI.getCardValue('idUsuario'));
				log.info('@@@ 2 GrupoCod: ' + hAPI.getCardValue('nomedoseugrupo2___' + indices[j]));
				log.info('@@@ 2 Manter: ' + hAPI.getCardValue('manterTwo___' + indices[j]));

				records[auxSun] = {};
				records[auxSun]['cod_usuario'] = '"' + hAPI.getCardValue('idUsuario') + '"';
				records[auxSun]['cod_grp_usuar'] = '"' + hAPI.getCardValue('nomedoseugrupo2___'+indices[j]) + '"';
				records[auxSun]['acao'] = (hAPI.getCardValue('manterTwo___'+indices[j]) == 'on') ? 'INC' : 'INC' ;
			}
