...
function getDescricaoProcesso() {
	...
	var c2 = DatasetFactory.createConstraint("processDefinitionPK.processId", getValue("WKDef"), getValue("WKDef"), ConstraintType.MUST);
	var constraints = new Array(c,c1,c2);
	var dados = DatasetFactory.getDataset('processDefinition', null, constraints, null).values

	if(dados == "undefined")
	{
        throw  "FALHA AO BUSCAR A DESCRICAO DO PROCESSO.";
    }
...
function startProcessFluigGcti(numAtividadeIntegracao, loginUsuario) 
{
	...
	try {
		...
		var retorno = hAPI.startProcess("FLUIG-0223", '597', ['adm'], "Solicitação aberta automaticamente por erro no processo " + getValue("WKDef"), true, p, false)
