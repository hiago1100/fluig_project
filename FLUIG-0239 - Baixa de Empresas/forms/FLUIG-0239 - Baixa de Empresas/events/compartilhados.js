...
ValidaFormulario.prototype.mensagem_de_erro = function()
{
    ...
    if (!this.formulario_valido) 
    {
        ...
        {
            this.error_message += 'OPA! problemas encontrados.<br>Favor verificar as seguintes informações:<br>';
...
ValidaFormulario.prototype.regra_CNPJ = function(config) 
{
    var valorCnpj = this.form.getValue(config.campo)
    var valorCnpjFilho = this.form.getValue(config.campo + "___" + config.index)
    
    var cnpj; 
    if (valorCnpj != '')
    {
        cnpj = valorCnpj;
    }else if(valorCnpjFilho != '')
    {
        cnpj = valorCnpjFilho;
    }

    cnpj = String(cnpj).replace(/[^\d]+/g,'');
    var mensagem = '<strong>o Campo "CNPJ" tem um número invalido.</strong>';
...
ValidaFormulario.prototype.regra_CNPJ_condicional = function(config) 
{

    if (!('condicoes' in config)) 
    {
        this.regra_CNPJ(config);
    } 
    else 
    {
        if(this.atende_condicoes(config.condicoes))
        ...
        else if(this.atende_condicoes_filho(config))
        {
            this.regra_CNPJ(config);
        }
    }
...
ValidaFormulario.prototype.regra_CPF_condicional = function(config) 
{
    
    if (!('condicoes' in config)) 
    {
        this.regra_CPF(config);
    } 
    else 
    {
