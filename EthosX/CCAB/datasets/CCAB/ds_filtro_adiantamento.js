	...
	try {
		...
		for (x = 0; x < retorno.getItem().size(); x++){
			...
			if (retorno.getItem().get(x).getStateId() == '42' ){
				...
				// log.info("##################################### 1 "+ aRetorno[0] + "/" + aRetorno[1]);	
	


			if(aRetorno[0] == "" || aRetorno[0] == null){
				cSaldo 	= "";
			}else{
				cSaldo 	= aRetorno[0];
			}	
			...
			if(aRetorno[1] == "" || aRetorno[1] == null){
				...
				// 	cFornecedor = "";
				// }
				cDtsolicitacao 	= trataData(dsDadosForm.getValue(0, "dataSolicitacao"));
				// if (cDtsolicitacao == null || cDtsolicitacao == "") {
				// 	cDtsolicitacao = "";
				// }
				cDtNecessidade 	= trataData(dsDadosForm.getValue(0, "dataNecessidade"));
				// }
				...
				// 	motivo = "";
				// }
				aprovador		= dsDadosForm.getValue(0, "matPrimeiroAprov");
	...
	try {
		...
		var columnCount = rs.getMetaData().getColumnCount();
		 ...
		 while (rs.next()) {
	        ...
	        if (null != obj) {
				saldo     = rs.getObject(rs.getMetaData().getColumnName(1)).toString();
				numTitulo = rs.getObject(rs.getMetaData().getColumnName(2)).toString();
	        	...
	        	return [saldo,numTitulo];
	        } else {
	            return "null";
			}
		 }
		 return [0,"Titulo não encontrado"];
	} catch (e) {
