    ...
    if (resulp) {
        ...
        if (records.length > 0) {
            for ( var index in records) {
                ...
                console.log("valor calculado "+ parseFloat(LIMITECALCULADO));
                ...
                console.log("VALOR DA MOEDA ERP = ", MOEDAERP);
                                           
                // var CHECK = checkAprovacoes(gestor, " "+SAFRA+" ", GRUPOVENDAS, checkManager()); 
                // if (!CHECK) {                                    
                    // var valor = formatterBr.format(LIMITECALCULADO);   PAREI AQUI ROLLBACK
                    ...
                    var rowNode = t.row.add( [                
                        ...
                        "<B>"+valor+"</B>"
                        // ,'<i class="fluigicon fluigicon-plus-sign details-size"></i>'
                    ] ).draw().node();        
                    vazio++;
            ...
            console.log("Valor sem trativa = "+ totalv);



            totalv = converteMoeda(MOEDAERP,totalv);
            
            console.log("Valor depois da tratativa = "+ totalv);
