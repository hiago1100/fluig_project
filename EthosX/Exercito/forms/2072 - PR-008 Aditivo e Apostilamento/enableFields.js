...
function enableFields(){
	...
	if(CONTEXT.CURRENT_STATE == Activity.ZERO || CONTEXT.CURRENT_STATE == Activity.INICIAR){
		enableContainer($("#panelValidaComposicao"),false);
		enableContainer($("#panelACODE"),false);
		enableContainer($("#panelGestaoContrato"),false);

	} else if(CONTEXT.CURRENT_STATE == Activity.VALIDAR_COMPOSICAO){
		enableContainer($("#panelDetalhesSolicitacao"),false);
		enableContainer($("#panelACODE"),false);
		enableContainer($("#panelGestaoContrato"),false);
		...
		enableContainer($("#panelValidaComposicao"),false);
		enableContainer($("#panelGestaoContrato"),false);
		enableContainer($("#divAssinaturasAditivo"),false);
		if($('input[name="rb_SM_de"]:checked').val() == 'aditivo'){
			enableContainer($("#divDatasAssinaturaPublicacoes"), false);
		}
		...
		enableContainer($("#panelValidaComposicao"),false);
		enableContainer($("#panelGestaoContrato"),false);
		enableContainer($("#divElaboracaoDocumemtalACODE"),false);
		
	} else if(CONTEXT.CURRENT_STATE == Activity.AVALIAR_MINUTA){
		enableContainer($("#panelDetalhesSolicitacao"),false);
		enableContainer($("#panelValidaComposicao"),false);
		enableContainer($("#panelACODE"),false);
		enableContainer($("#divAtualizarContrato"),false);
	
	} else if(CONTEXT.CURRENT_STATE == Activity.ATUALIZAR_CONTRATO){
		enableContainer($("#panelDetalhesSolicitacao"),false);
		enableContainer($("#panelValidaComposicao"),false);
		enableContainer($("#panelACODE"),false);
		enableContainer($("#divAvalicaoMinuta"),false);	
	 
	} else if(CONTEXT.CURRENT_STATE == Activity.ELABORAR_ANALISES){
		enableContainer($("form")[0],false);
		$("#desc_finalidade").removeAttr('readonly'); // Finalidade
		$("#vl_aditivo_apost").removeAttr('readonly'); // Valor
		$("#vl_diferenca").removeAttr('readonly'); // Diferença 
		$("#vl_variacao").removeAttr('readonly'); // Variação 	
	} else {		
		enableContainer($("form")[0],false);
	}	
}
