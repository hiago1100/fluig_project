...
ValidaFormulario.prototype.mensagem_de_erro = function()
{
    ...
    if (!this.formulario_valido) 
    {
        if (this.campos_nao_preenchidos.length > 0) 
        {
            ...
            this.error_message += campos_para_preencher.join(' ,') + "<br>";
        }
    }
...
ValidaFormulario.prototype.regra_aba_filho_obrigatorio = function(config) 
{
    if (this.form.getValue(config.campo + "___" + config.index) == '') 
    {
        this.formulario_valido = false;
        this.errors.push('O campo "' + config.label + '" da aba "' + config.label_pai + ' #' + config.index +'" não está preenchido!' );
    }
};
