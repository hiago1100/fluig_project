...
ValidaFormulario.prototype.regra_filho_obrigatorio = function(config) 
{
    ...
    {
        this.formulario_valido = false;
        this.errors.push("O campo " + config.label + '" para "' + config.label_pai + '" não preenchido!' );
    }
};
...
ValidaFormulario.prototype.regra_filho_obrigatorio_condicional = function(config) {
    if (this.form.getValue(config.campo + "___" + config.index) == '') {
    	if (this.form.getValue(config.campoCondicao) != 1 && this.form.getValue(config.campoCondicao) != 2 ) {
    		this.formulario_valido = false;
            this.errors.push('Campo "' + config.label + '" para "' + config.label_pai + '" não preenchido!');
    	}
    }
};
