...
var ARYForms = {
    ...
    $(function () {
      if (params.formMode == "ADD" || params.formMode == "MOD") { 
        ...
        onView: function(params) { //Visualização do formulário sem a possibilidade de edição (consulta)
          ...
          var cod_grupo = $("#codGrupo").text();

          carregaUsuarios(cod_grupo);


          var dadoss2 = '';
          cod_grupo_revisao =  $("#codDoGrupo").text();
  ...
  function carregaProgramas(cod_grupo){
   ...
   if (cod_grupo != ""){  
    ...
    var dbInstance =  $("#tipoBanco").val();

    var dados = {"name": "SGA-sql2dataset-programa","fields":null,"constraints":[
    {"_field":"idGrupo","_initialValue":cod_grupo,"_finalValue": cod_grupo,"_type":1}
    ]};
    $.ajax({
     method: "POST",
     url: "/api/public/ecm/dataset/datasets/",
   ...
   success:function(model) {
    ...
    $.each(model.content.values, function(index, value){
     ...
     var codigo_program = value.cod_programa;
     var descricao_Program = value.descricao_programa;
     var idGrupoPk = value.z_sga_programas_id

     $("#nomeProgAux___"+ultimaLinha).val(codigo_program);
     $("#descProgAux___"+ultimaLinha).val(descricao_Program);

     $("#idGrupoPk___"+ultimaLinha).val(idGrupoPk);
...
function carregaUsuarios(cod_grupo_revisao){
  ...
  console.log("nome do meu Grupo", cod_grupo_revisao);
  ...
  if (cod_grupo_revisao != ""){ 
    ...
    console.log("INSTANCIA DO BANCO ."+dbInstance);

    var dados = {"name": "SGA-sql2dataset-usuario-grupos","fields":null,"constraints":[
    {"_field":"idGrupoUser","_initialValue":cod_grupo_revisao,"_finalValue": cod_grupo_revisao,"_type":1},
    {"_field":"dbInstance","_initialValue":dbInstance,"_finalValue": dbInstance,"_type":1}
    ]};
...
function setSelectedZoomItem(selectedItem) {     
  ...
  if(selectedItem.inputName == "nomeEmpresa") {
    ...
    console.log("ID DA EMPRESA"+idEmpresa);
      ...
      reloadZoomFilterValues("codGrupo", "codigoEmpresa," + idEmpresa);  
     //   reloadZoomFilterValues("codUsuario", "idEmpresa," + idEmpresa);

     $(".controleEmpresa").removeClass("hide");

   } else if(selectedItem.inputName == 'codGrupo'){  


    $('input[id^="nomeProgAux___"]').parent().parent('tr').remove();
    $('input[id^="appConflitoC___"]').parent().parent('tr').remove();         
    $('#nomeGrupo').val(selectedItem['descAbrev']);
    $('#gestorGrupo').val("Hiago Oliveira");
    $("#recebeCod").val("admin");
    $("#codGest").val(selectedItem["GESTOR"]);

    $("#codGrupoAux").val(selectedItem["idLegGrupo"]);

    var cod_grupo = selectedItem["idLegGrupo"];
        //carregaScroll();
        ...
        carregaTela(cod_grupo,cod_grupo); 
        carregaProgramas(cod_grupo);
       // carregaProgramasAll(cod_grupo);
       carregaUsuariosAll(cod_grupo);


//        reloadZoomFilterValues("addGrupo", "idGrupo," + cod_grupo);

     //   reloadZoomFilterValues("removeGrupoDesc", "idGrupo," + cod_grupo);
    ...
    var cod_grupo = selectedItem['cod_grupo'];
     ...
     } else if(selectedItem.inputName == "programaInc") {
  ...
  var linha = wdkAddChild('tabela_programas3');

  $("#descProgAuxTwo___"+linha).val(selectedItem["descricao_programa"]);
  $("#nomeProgAuxTwo___"+linha).val(selectedItem["cod_programa"]);
  $("#nomeGestProg___"+linha).val("Hiago Oliveira");
  $("#codGestProg___"+linha).val("hiago.domingos");
  $("#idProgPk___"+linha).val(selectedItem["z_sga_programas_id"]);
...
//             $("#codGestorModuloTwo___"+linha).val("hiago.domingos");

// //            $("#recebeCod1").val("hiago.domingos");
...
function fnCustomDelete(oElement){
  ...
  function concatenaGest(){
    ...
    $('input[id^="descProgAuxTwo___"]').each(function(x){
      ...
      if($("#codGestProg___"+linha).val() != ""){

        arr1.push($("#codGestProg___"+linha).val());          
      }       
    });
...
function carregaParametros(){
        ...
        var idPastaMae = 171;
        var timeBusca = {"limit" : 999}

        $.ajax({
            method: "GET",
            url: "/api/public/ecm/document/listDocumentWithChildren/"+idPastaMae,
            data: timeBusca,
            contentType: "application/json", 
            async: false,
            error: function(x, e) {
           ...
           success:function(model) {      
               ...
               $.each(model.content[0].children, function(index, value){
                   ...
                   var dbInstance = dadosarray[1];
                   $("#tipoBanco").val(dbInstance);
