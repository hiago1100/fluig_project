function iniciaCadastroFornecedor(fornecedor, datasetMunicipios){
	log.info("### iniciaCadastroFornecedor inicio");
	...
	var solicitante = hAPI.getCardValue("txtNmRequisitante");
	var cdSolicitante = hAPI.getCardValue("cdSolicitante");
    ...
    var codigo_filial = hAPI.getCardValue("FilialAlcada");
	/*
	 * Inscrição Estadual
	 * 
	 */
	...
	if(fornecedor.has("Telefone") && fornecedor.get("Telefone") != ""){
		...
		if(telefone.indexOf("(") > -1){
			...
			ddd = ddd.replace(")","");
			if(telefone.indexOf(" ") > -1){
				telefone = telefone.substring(telefone.indexOf(" ")+1,(telefone+"").length);
			}else{
				telefone = telefone.substring(telefone.indexOf(")")+1,(telefone+"").length);
			}
	...
	if(fornecedor.has("Fax") && fornecedor.get("Fax") != ""){
		...
		if(fax != null) {
			if(fax.indexOf("(") > -1){
				if(fax.indexOf(" ") > -1){
					fax = fax.substring(fax.indexOf(")")+1,(fax+"").length);
				}else{
					fax = fax.substring(fax.indexOf(" ")+1,(fax+"").length);
				}
    ...
    try {
	    ...
	    var keyValueDto24 = workflowServiceProvider.instantiate("com.totvs.technology.ecm.workflow.ws.KeyValueDto");
	    keyValueDto24.setKey("solicitante");
	    keyValueDto24.setValue(solicitante);
	    listaCampos.push(keyValueDto24);
	    
	    var keyValueDto25 = workflowServiceProvider.instantiate("com.totvs.technology.ecm.workflow.ws.KeyValueDto");
	    keyValueDto25.setKey("cdSolicitante");
	    keyValueDto25.setValue(cdSolicitante);
	    listaCampos.push(keyValueDto25);
	    
	    var keyValueDto26 = workflowServiceProvider.instantiate("com.totvs.technology.ecm.workflow.ws.KeyValueDto");
	    keyValueDto26.setKey("datasolic");
	    keyValueDto26.setValue(dataAtual);
	    listaCampos.push(keyValueDto26);
	    
	    var keyValueDto27 = workflowServiceProvider.instantiate("com.totvs.technology.ecm.workflow.ws.KeyValueDto");
	    keyValueDto27.setKey("filial_protheus");
	    keyValueDto27.setValue(filial_protheus);
	    listaCampos.push(keyValueDto27);
	    
	    var keyValueDto28 = workflowServiceProvider.instantiate("com.totvs.technology.ecm.workflow.ws.KeyValueDto");
	    keyValueDto28.setKey("codigo_filial");
	    keyValueDto28.setValue(codigo_filial);
	    listaCampos.push(keyValueDto28);
	    
	    keyValueDtoArray.setItem(listaCampos);
	    log.info("### Antes startProcessClassic");
	    ...
	    var keyValueDtoArrayResult = workflowService.startProcessClassic(wsUser, wsPass, wsCompany, PROCESSO_CADASTRO_FORNECEDOR, new java.lang.Integer(40), users, "Processo Iniciado através da solicitação de compras "+getValue("WKNumProces"), matriculaResponsavel, false, processAttachmentDtoArray, keyValueDtoArray, processTaskAppointmentDtoArray, false);
    ...
    log.info("### iniciaCadastroFornecedor fim");
}
