////////////////////////////////////////////////////////////////////////////////
/*ARQUIVO COM FUNÇÕES COMPARTILHADAS NO BACKEND DO FORMULARIO  V1.1 */
//Data ultima atualização: 01/03/2019
////////////////////////////////////////////////////////////////////////////////
...
ValidaFormulario.prototype.obriga_campo_pai_filho = function(config) 
{
...
ValidaFormulario.prototype.atende_condicoes_pai_filho = function(config) {
    for (let i = 0; i < config.condicoes.length; i++) {
        
        var valor_campo = this.form.getValue(config.condicoes[i].campo+'___'+config.index);
        
        for (let j = 0; j < config.condicoes[i].valores.length; j++) {
            if (config.condicoes[i].valores[j] == valor_campo) {
                return true;
            }
        }
    }
...
ValidaFormulario.prototype.regra_minimo_de_filhos = function(minFilhos) 
{
    return function(config) 
    {
        var indexes = this.form.getChildrenIndexes(config.tablename);
        log.dir('INDEX-->' + indexes.length);
        if (indexes.length < minFilhos) {
            this.formulario_valido = false;
            this.errors.push("Adicione ao menos " + minFilhos + ' "' + config.label + '" <strong>(' + (minFilhos - indexes.length) + " faltando)</strong>!");
        }
    };
}
...
ValidaFormulario.prototype.regra_filho_obrigatorio = function(config) 
{
    if (!('condicoes' in config)) 
    {
        this.obriga_campo_pai_filho(config);
    } 
    else 
    {
        if(this.atende_condicoes_pai_filho(config))
        {
            this.obriga_campo_pai_filho(config);
        }
    }
