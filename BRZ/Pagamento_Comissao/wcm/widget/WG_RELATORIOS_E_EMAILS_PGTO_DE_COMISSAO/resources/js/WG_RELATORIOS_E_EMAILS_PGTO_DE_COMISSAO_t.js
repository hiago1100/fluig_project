...
function consultDB(imob, type){
    ...
    if(type == "consolidado"){
        // consult  = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_BRUTO" 
        // consult  = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_PPARC" 
        consult  = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_SPARC"
    }
...
function updateTableConsolidated(obj, type){
    ...
    if(type == "bruto"){
        query = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_BRUTO";
    }else if(type == "parc1"){
        query = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_PPARC";
    }else if(type == "parc2"){
        query = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_SPARC";
    }else if(type == "parcU"){
        query = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_PARCU";
    }else if(type == "dist"){
        query = "SELECT * FROM FLUIG.DBO.FLUIG_CONSOLIDADOS_PARCD";
    }
      ...
      success : function(data){
        ...
        if(type == "bruto"){
            ...
            if((arrDtIni.length == 0 && arrDtFim.length == 0) || (arrDtIni.indexOf(obj.ini) === -1 && arrDtFim.indexOf(obj.fim) === -1)){
                qry  = " INSERT INTO FLUIG.DBO.FLUIG_CONSOLIDADOS_BRUTO  (ID, PeriodoIni, PeriodoFin, Imobiliaria, Empreendimento, ValorBruto, Desconto, Despesas, ValorNF)";
            ...
            for(i=0; i<data.content.values.length; i++){
                ...
                if(arrID.indexOf(obj.ID) === -1){
                    qry  = " INSERT INTO FLUIG.DBO.FLUIG_CONSOLIDADOS_SPARC  (ID, DataVenda, DataAgregacao, NumVenda, Empreendimento, Bloco, Unidade, NomeCliente, NomeImobiliaria, NomeCorretor, TipoVenda, ValorBase, ValorSegParcCom, ValorParcDemMin, ValorParcDemMax, ValorParcBoni)";
            ...
            for(i=0; i<data.content.values.length; i++){
                ...
                if(arrID.indexOf(obj.ID) === -1){
                    qry  = " INSERT INTO FLUIG.DBO.FLUIG_CONSOLIDADOS_PARCU  (ID, DataVenda, NumVenda, Empreendimento, Bloco, Unidade, NomeCliente, NomeImobiliaria, NomeCorretor, TipoVenda, ValorBase, ValorPriParcCom, ValorParcBoni, TotalComissao, TotalBonificacao)";
            ...
            for(i=0; i<data.content.values.length; i++){
                ...
                if(arrID.indexOf(obj.ID) === -1){
                    qry  = " INSERT INTO FLUIG.DBO.FLUIG_CONSOLIDADOS_PARCD  (ID, DataVenda, NumVenda, Empreendimento, Bloco, Unidade, NomeCliente, NomeImobiliaria, NomeCorretor, TipoVenda, ValorBase, ValorPriParcCom, ValorParcBoni, TotalComissao, TotalBonificacao)";
                    qry += " VALUES('"+obj[i].ID+"', '"+obj[i].NmImob+"', '"+obj[i].DtVenda+"', '"+obj[i].NuVenda+"', '"+obj[i].Emp+"', '"+obj[i].Bl+"', '"+obj[i].Uni+"', '"+obj[i].NmCliente+"', '"+obj[i].NmCorretor+"', '"+obj[i].TpVenda+"', '"+obj[i].VlBase+"', '"+obj[i].VlPriParcCom+"', '"+obj[i].VlParcBoni+"', '"+obj[i].TotComissao+"', '"+obj[i].TotBonificacao+"')";
                }else{
